const body = document.body;
const searchWrapper = document.querySelector(".search-wrapper");
const searchModal = document.querySelector(".search-modal");
const searchFooter = document.querySelector(".search-wrapper-footer");
const searchResult = document.querySelectorAll("[data-search-result]");
const searchResultItemTemplate = document.getElementById(
  "search-result-item-template"
);
const hasSearchWrapper = searchWrapper != null;
const hasSearchModal = searchModal != null;
const searchInput = document.querySelectorAll("[data-search-input]");
const emptySearchResult = document.querySelectorAll(".search-result-empty");
const openSearchModal = document.querySelectorAll(
  '[data-target="search-modal"]'
);
const closeSearchModal = document.querySelectorAll(
  '[data-target="close-search-modal"]'
);
const searchIcon = document.querySelector(
  ".search-wrapper-header label svg[data-type='search']"
);
const searchIconReset = document.querySelector(
  ".search-wrapper-header label svg[data-type='reset']"
);
const searchResultInfo = document.querySelector(".search-result-info");
let searchModalVisible =
  hasSearchModal && searchModal.classList.contains("show") ? true : false;
let jsonData = [];

// Pagination State
let currentResultsLimit = 20;
let lastSearchQuery = "";
const searchLoadMoreContainer = document.getElementById("search-load-more");
const searchLoadMoreBtn = searchLoadMoreContainer ? searchLoadMoreContainer.querySelector("button") : null;

// Keyboard Navigation State
let selectedIndex = -1;
let currentResItems = [];

const loadJsonData = async () => {
  try {
    // indexURL is defined globally in the HTML
    const res = await fetch(indexURL);
    return (jsonData = await res.json());
  } catch (err) {
    console.error("Failed to load search index:", err);
  }
};

// escape HTML entities
function escapeHTML(input) {
  return input
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
}

// escape RegExp special characters
function escapeRegExp(string) {
  return string.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

const capitalizeFirstLetter = (string) => {
  return string
    .replace(/^[\s_]+|[\s_]+$/g, "")
    .replace(/[_\s]+/g, " ")
    .replace(/^[a-z]/, function (m) {
      return m.toUpperCase();
    });
};

const slugify = (string) => {
  let lowercaseText = string
    .trim()
    .replace(/[\s_]+/g, "-")
    .toLowerCase();
  return encodeURIComponent(lowercaseText);
};

const renderResult = (templateString, data) => {
  var conditionalMatches, conditionalPattern, copy;
  conditionalPattern = /\#\{\s*isset ([a-zA-Z]*) \s*\}(.*)\#\{\s*end\s*}/g;
  copy = templateString;
  while (
    (conditionalMatches = conditionalPattern.exec(templateString)) !== null
  ) {
    if (data[conditionalMatches[1]]) {
      copy = copy.replace(conditionalMatches[0], conditionalMatches[2]);
    } else {
      copy = copy.replace(conditionalMatches[0], "");
    }
  }
  templateString = copy;
  var key, find, re;
  for (key in data) {
    find = "\\#\\{\\s*" + key + "\\s*\\}";
    re = new RegExp(find, "g");
    templateString = templateString.replace(re, data[key]);
  }
  return templateString;
};

const displayResult = (searchItems, searchString) => {
  if (!searchWrapper) return;
  const image = searchWrapper.getAttribute("data-image");
  const description = searchWrapper.getAttribute("data-description");
  const tags = searchWrapper.getAttribute("data-tags");
  const categories = searchWrapper.getAttribute("data-categories");

  let allHits = [];
  searchItems.forEach(group => {
    const filtered = group.data.filter(d =>
      d.title.toLowerCase().includes(searchString) ||
      (description === "true" ? d.description?.toLowerCase().includes(searchString) : "") ||
      d.searchKeyword.toLowerCase().includes(searchString) ||
      (tags === "true" ? d.tags?.toLowerCase().includes(searchString) : "") ||
      (categories === "true" ? d.categories?.toLowerCase().includes(searchString) : "") ||
      d.content.toLowerCase().includes(searchString)
    );
    filtered.forEach(item => {
      allHits.push({ section: group.section, item });
    });
  });

  const totalHits = allHits.length;
  const visibleHits = allHits.slice(0, currentResultsLimit);

  const groupedVisible = [];
  visibleHits.forEach(hit => {
    let group = groupedVisible.find(g => g.section === hit.section);
    if (!group) {
      group = { section: hit.section, data: [] };
      groupedVisible.push(group);
    }
    group.data.push(hit.item);
  });

  const generateSearchResultHTML = (group) => {
    const highlightResult = (content) => {
      if (!content) return "";
      const regex = new RegExp(escapeRegExp(searchString), "gi");
      return content.replace(regex, (match) => `<mark>${match}</mark>`);
    };

    const highlightResultContent = (content) => {
      if (!content) return "";
      const regex = new RegExp(escapeRegExp(searchString), "gi");
      const matchIndex = content.search(regex);

      if (matchIndex >= 0) {
        const contextBefore = 5;
        const contextAfter = 20;
        const start = Math.max(0, matchIndex - contextBefore);
        const end = Math.min(content.length, matchIndex + searchString.length + contextAfter);

        let snippet = content.substring(start, end);
        snippet = snippet.replace(regex, (match) => `<mark>${match}</mark>`);

        if (start > 0) snippet = "..." + snippet;
        if (end < content.length) snippet = snippet + "...";
        return snippet;
      }
      return content.substring(0, 120) + (content.length > 120 ? "..." : "");
    };

    let templateDefinition = searchResultItemTemplate != null ? searchResultItemTemplate.innerHTML : `
      <div class="search-result-item">
        <div class="search-result-item-body">
          <a href="#{slug}" class="search-result-item-title">#{title}</a>
          <p class="search-result-item-content">#{content}</p>
        </div>
      </div>`;

    const renderedItems = group.data.map((innerItem) => {
      return renderResult(templateDefinition, {
        slug: innerItem.slug,
        date: innerItem.date,
        description: description == "true" ? highlightResult(innerItem.description) : "",
        title: highlightResult(innerItem.title),
        image: image == "true" ? innerItem.image : "",
        tags: tags == "true" ? highlightResult(innerItem.tags) : "nomatch",
        categories: categories == "true" ? highlightResult(innerItem.categories) : "nomatch",
        content: highlightResultContent(innerItem.content),
      });
    }).join("");

    return `
      <div class="search-result-group">
        <p class="search-result-group-title">${group.section}</p>
        ${renderedItems}
      </div>`;
  };

  const startTime = performance.now();
  const htmlString = groupedVisible.map(generateSearchResultHTML).join("");
  searchResult.forEach((el) => {
    el.innerHTML = htmlString;
  });
  const endTime = performance.now();

  let totalResults = `<em>${totalHits}</em> results`;
  let totalTime = ((endTime - startTime) / 1000).toFixed(3);
  totalTime = `- in <em>${totalTime}</em> seconds`;

  searchResultInfo &&
    (searchResultInfo.innerHTML =
      totalHits > 0 ? `${totalResults} ${totalTime}` : "");

  if (searchLoadMoreContainer) {
    if (currentResultsLimit < totalHits) {
      searchLoadMoreContainer.classList.remove("hidden");
    } else {
      searchLoadMoreContainer.classList.add("hidden");
    }
  }

  currentResItems = searchResult[0] ? searchResult[0].querySelectorAll(".search-result-item") : [];
  selectItem(-1);

  // Manage Empty Search Result visibility
  if (totalHits > 0) {
    emptySearchResult.forEach((es) => (es.style.display = "none"));
  } else {
    emptySearchResult.forEach((es) => (es.style.display = "block"));
  }

  const groupTitle = document.querySelectorAll(".search-result-group-title");
  groupTitle.forEach((el) => {
    if (el.nextElementSibling === null) {
      el.style.display = "none";
    }
  });

  const searchInfo = document.querySelectorAll(".search-info > div");
  if (searchInfo.length > 0) {
    searchInfo.forEach((el) => {
      if (el.innerText.includes("nomatch") || el.innerText == "") {
        el.classList.add("hidden");
      }
    });
  }
};

const selectItem = (index) => {
  if (index >= 0 && index < currentResItems.length) {
    for (let i = 0; i < currentResItems.length; i++) {
      currentResItems[i].classList.toggle("search-item-selected", i === index);
    }
    selectedIndex = index;
    currentResItems[index].scrollIntoView({
      behavior: "auto",
      block: "nearest",
    });
  } else if (index === -1) {
    for (let i = 0; i < currentResItems.length; i++) {
      currentResItems[i].classList.remove("search-item-selected");
    }
    selectedIndex = -1;
  }
};

const handleKeyDown = (event) => {
  if (currentResItems.length !== 0) {
    if (event.key === "ArrowUp" || event.key === "ArrowDown") {
      event.preventDefault();
      if (event.key === "ArrowUp") {
        selectedIndex = selectedIndex > 0 ? selectedIndex - 1 : currentResItems.length - 1;
      } else if (event.key === "ArrowDown") {
        selectedIndex = selectedIndex < currentResItems.length - 1 ? selectedIndex + 1 : 0;
      }
      selectItem(selectedIndex);
    } else if (event.key === "Enter") {
      if (selectedIndex !== -1) {
        event.preventDefault();
        let selectedLink = currentResItems[selectedIndex]
          .getElementsByClassName("search-result-item-title")[0]
          .getAttribute("href");
        window.location.href = selectedLink;
      }
    }
  }
};

const doSearch = async (searchString) => {
  if (searchString !== lastSearchQuery) {
    currentResultsLimit = 20;
    lastSearchQuery = searchString;
  }
  if (searchString !== "") {
    searchIcon && (searchIcon.style.display = "none");
    searchIconReset && (searchIconReset.style.display = "initial");
    emptySearchResult.forEach((el) => {
      const notFoundContent = `
        <div class="search-not-found">
          <svg width="42" height="42" viewBox="0 0 47 47" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M7.10368 33.9625C9.90104 36.2184 13.2988 37.6547 16.9158 38.0692C21.6958 38.617 26.5063 37.3401 30.3853 34.4939C30.4731 34.6109 30.5668 34.7221 30.6721 34.8304L41.9815 46.1397C42.5323 46.6909 43.2795 47.0007 44.0587 47.001C44.838 47.0013 45.5854 46.692 46.1366 46.1412C46.6878 45.5904 46.9976 44.8432 46.9979 44.064C46.9981 43.2847 46.6888 42.5373 46.138 41.9861L34.8287 30.6767C34.7236 30.5704 34.6107 30.4752 34.4909 30.3859C37.3352 26.5046 38.6092 21.6924 38.0579 16.912C37.6355 13.2498 36.1657 9.81322 33.8586 6.9977L31.7805 9.09214C34.0157 11.9274 35.2487 15.4472 35.2487 19.0942C35.2487 21.2158 34.8308 23.3167 34.0189 25.2769C33.207 27.2371 32.0169 29.0181 30.5167 30.5184C29.0164 32.0186 27.2354 33.2087 25.2752 34.0206C23.315 34.8325 21.2141 35.2504 19.0925 35.2504C16.9708 35.2504 14.8699 34.8325 12.9098 34.0206C11.5762 33.4682 10.3256 32.7409 9.18992 31.8599L7.10368 33.9625ZM28.9344 6.28152C26.1272 4.12516 22.671 2.93792 19.0925 2.93792C14.8076 2.93792 10.6982 4.64009 7.66829 7.66997C4.6384 10.6999 2.93623 14.8093 2.93623 19.0942C2.93623 21.2158 3.35413 23.3167 4.16605 25.2769C4.72475 26.6257 5.4625 27.8897 6.35716 29.0358L4.2702 31.1391C1.35261 27.548 -0.165546 23.0135 0.00974294 18.3781C0.19158 13.5695 2.18233 9.00695 5.58371 5.60313C8.98509 2.19932 13.5463 0.205307 18.3547 0.0200301C22.9447 -0.156832 27.4369 1.32691 31.0132 4.18636L28.9344 6.28152Z" fill="currentColor"/>
            <path d="M3.13672 39.1367L38.3537 3.64355" stroke="black" stroke-width="3" stroke-linecap="round"/>
          </svg>
          <p>${typeof search_no_results !== 'undefined' ? search_no_results : "No results for"} "<b>${escapeHTML(searchString)}</b>"</p>
        </div>`;
      el.innerHTML = notFoundContent;
    });
  } else {
    searchIcon && (searchIcon.style.display = "initial");
    searchIconReset && (searchIconReset.style.display = "none");
    emptySearchResult.forEach((el) => {
      el.innerHTML = typeof search_initial_message !== 'undefined' ? search_initial_message : "Type to search...";
    });
    if (searchLoadMoreContainer) searchLoadMoreContainer.classList.add("hidden");
  }

  // Global variables: includeSectionsInSearch
  if (typeof includeSectionsInSearch === 'undefined') return;

  let filteredJSON = includeSectionsInSearch.map((section) => {
    const data = jsonData.filter(
      (item) => slugify(item.section) === slugify(section)
    );
    const sectionName = section.replace(/[-_]/g, " ");
    return {
      section: capitalizeFirstLetter(sectionName),
      data,
    };
  });

  let searchItem = filteredJSON.filter((item) => {
    if (searchString === "") return false;
    return item.data.some((el) => {
      const regex = new RegExp(escapeRegExp(searchString), "gi");
      return (
        el.title.toLowerCase().match(regex) ||
        el.description?.toLowerCase().match(regex) ||
        el.searchKeyword.toLowerCase().match(regex) ||
        el.content.toLowerCase().match(regex) ||
        el.tags?.toLowerCase().match(regex) ||
        el.categories?.toLowerCase().match(regex)
      );
    });
  });

  displayResult(searchItem, searchString);
};

if (hasSearchWrapper) {
  searchInput.forEach((el) => {
    el.addEventListener("keypress", (e) => {
      if (e.keyCode == 13) e.preventDefault();
    });
    el.addEventListener("input", (e) => {
      const q = e.target.value.toLowerCase();
      window.history.replaceState({}, "", `${window.location.origin}${window.location.pathname}?s=${q.replace(/ /g, "+")}`);
      doSearch(q);
    });
    el.addEventListener("keydown", handleKeyDown);
  });

  document.addEventListener("DOMContentLoaded", async () => {
    await loadJsonData();
    const urlParams = new URLSearchParams(window.location.search);
    const q = urlParams.get("s") || "";
    if (q) doSearch(decodeURIComponent(q.replace(/\+/g, " ")));
  });

  if (searchLoadMoreBtn) {
    searchLoadMoreBtn.addEventListener("click", () => {
      currentResultsLimit += 20;
      doSearch(lastSearchQuery);
    });
  }

  searchIconReset && searchIconReset.addEventListener("click", () => resetSearch());
  openSearchModal.forEach((el) => el.addEventListener("click", () => showModal()));
  closeSearchModal.forEach((el) => el.addEventListener("click", () => closeModal()));
  searchWrapper.addEventListener("click", (e) => { if (e.target.classList.contains("search-wrapper")) closeModal(); });
  document.addEventListener("keydown", (e) => {
    if ((e.ctrlKey || e.metaKey) && e.key === "k") {
      e.preventDefault();
      searchModalVisible ? closeModal() : showModal();
    }
    if (e.key === "Escape" && searchModalVisible) {
      e.preventDefault();
      closeModal();
    }
  });
}

const resetSearch = () => {
  searchIcon && (searchIcon.style.display = "initial");
  searchIconReset && (searchIconReset.style.display = "none");
  searchInput.forEach((el) => { el.value = ""; });
  searchResult.forEach((el) => { el.innerHTML = ""; });
  emptySearchResult.forEach((el) => {
    el.style.display = "";
    el.innerHTML = typeof search_initial_message !== 'undefined' ? search_initial_message : "";
  });
  if (searchResultInfo) searchResultInfo.innerHTML = "";
  if (searchLoadMoreContainer) searchLoadMoreContainer.classList.add("hidden");
  if (window.location.search.includes("?s=")) {
    window.history.pushState("", document.title, window.location.pathname + window.location.hash);
  }
};

const enableBodyScroll = () => { setTimeout(() => { body.style.overflowY = ""; body.style.paddingRight = ""; }, 200); };
const disableBodyScroll = () => {
  const scrollbarWidth = Math.abs(window.innerWidth - document.documentElement.clientWidth);
  body.style.overflowY = "hidden";
  body.style.paddingRight = scrollbarWidth + "px";
};
const showModal = () => {
  searchModal.classList.add("show");
  window.setTimeout(() => {
    const firstInput = document.querySelector("[data-search-input]");
    if (firstInput) firstInput.focus();
  }, 100);
  disableBodyScroll();
  searchModalVisible = true;
};
const closeModal = () => {
  searchModal.classList.remove("show");
  resetSearch();
  enableBodyScroll();
  searchModalVisible = false;
};
