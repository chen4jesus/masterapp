...
export class Audios extends Component {
    ...
    reloadList() {
        this.stopEdit();
        window.$http.get(`/audios/get/page/${this.pageId}`).then(resp => {
    ...
    updateOrder(idOrder) {
        window.$http.put(`/audios/sort/page/${this.pageId}`, {order: idOrder}).then(resp => {
    ...
    async startEdit(id) {
        ...
        const resp = await window.$http.get(`/audios/edit/${id}`);
