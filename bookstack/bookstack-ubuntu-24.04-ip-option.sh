...
# Fetch domain to use from the first provided parameter,
# Otherwise, request the user to input their domain
function run_prompt_for_domain_if_required() {
  if [ -z "$DOMAIN" ]
  then
    # Attempt to retrieve the current machine's IP address
    CURRENT_IP=$(hostname -I | awk '{print $1}')
    if [ -z "$CURRENT_IP" ]
    then
      error_out "Could not determine the server's IP address. Please provide a domain or IP address as an argument."
    else
      DOMAIN="$CURRENT_IP"
    fi
  fi
  ...
  info_msg ""
  info_msg "Using domain or IP: $DOMAIN"
  info_msg ""
}
